...
def cluster_model(params, gpu, input_clustering, feature_clustering):
    ...
    if not input_clustering and not feature_clustering:
        raise Exception("Need to specify at least on clusering, otherwise select base model.")
    ...
    for n_clusters_per_class_input in params.num_clusters_per_class_input:
        ...
        logger.info(f"Using Cluster Model with Input Clustering={input_clustering} and Feature Clustering={feature_clustering}")

        if input_clustering:
            train_data, validation_data, test_data = init_datasets(params, n_clusters_per_class_input)
        else:
            train_data, validation_data, test_data = init_datasets(params, 1)


        train_data_loader, validation_data_loader, test_data_loader = init_dataloader(train_data, validation_data, test_data)
        ...
        optimizer = torch.optim.SGD(
            cluster_model.parameters(), lr=params.learning_rate, momentum=params.momentum
        )

        if input_clustering:
            path_model = params.saved_models_dir + f"openmax_cnn_eminst_cluster-{total_n_clusters}.pth"
        else:
        ...
        if not params.train_only:
            ...
            for n_clusters_per_class_features in params.num_clusters_per_class_features:
                for alpha in params.alphas:
                    ...
                    known_unknown_acc(openmax_predictions_per_model, n_clusters_per_class_input)

                    ccr_fpr_per_model = oscr(openmax_scores_per_model)
                    
                    save_oscr_values(params.eperiment_data_dir,"base", n_clusters_per_class_features, ccr_fpr_per_model, alpha, params.negative_fix)
